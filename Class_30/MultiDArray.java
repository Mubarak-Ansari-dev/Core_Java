...
public class MultiDArray{
    public static void main (String args[]){
        ...
        int nums[][] = new int[3][4]; // 3 rows and 4 columns
           ...
           //printing random values in 2d array
        
        for(int i = 0; i<3; i++){   //traversing rows
            for(int j=0; j<4; j++){  //traversing columns

                nums[i][j] = (int)(Math.random()*10); // assigning random value between 0 to 9  
            }
        }
        ...
        for(int i = 0; i<3; i++){   //traversing rows
            for(int j=0; j<4; j++){  //traversing columns
                ...
                System.out.print(nums[i][j] + " "); // default value 0
            }
            System.out.println();
        }

        //using enhanced for loop to traverse 2d array
        for(int n[]: nums){
            for(int m: n){ // here m is element of single dimension array n
                System.out.print(m + " ");
            }
            System.out.println();
        }
