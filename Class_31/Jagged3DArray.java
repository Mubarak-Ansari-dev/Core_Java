...
#1
Suppose we want to create a 2 dimensional array such that it has 
5 rows but we don't know for each row we need different size for column.
...
public class Jagged3DArray {
    ...
    public static void main(String args[]){
        ...
        int nums[][] = new int[3][]; //jagged array creation
        ...
        for(int i=0; i<nums.length; i++){
            for(int j=0; j<nums[i].length; j++){
                nums[i][j] = (int)(Math.random()*10); // assingning random value between 0 to 9
            }
        }
